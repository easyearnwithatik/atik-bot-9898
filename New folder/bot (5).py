...
# Tracks pending number assignment per user: number, message_id, otp status
_user_pending: dict[int, dict] = {}
# uid -> {"number": str, "num_msg_id": int|None, "has_otp": bool, "svc": str, "scnt": str}
...
def send_otp_message(chat_id, otp, number, seconds, service=""):
    ...
    if chat_id == get_otp_group_id():
        ...
        # Mark has_otp so "Get New Number" knows OTP arrived for this user
        if uid in _user_pending:
            _user_pending[uid]["has_otp"] = True
        # Delete the "Number Assigned" message when OTP arrives
        prev_msg_id = (
            (_user_pending.get(uid) or {}).get("num_msg_id")
            or _user_last_num_msg.get(uid)
        )
        if prev_msg_id:
            try:
...
def callback_handler(call):
    ...
    try:
        ...
        if data == "v":
            ...
            pending = _user_pending.get(uid_n, {})

            if pending and not pending.get("has_otp", True):
                # ── Pending number, no OTP yet → CANCEL: restore to stock + delete msg ──
                old_num = pending.get("number", "")
                if old_num:
                    old_clean = re.sub(r"\D", "", str(old_num))
                    with user_map_lock:
                        user_map.pop(old_clean, None)
                        assigned_time.pop(old_clean, None)
                    _save_user_map()
                    p_svc = pending.get("svc", svc)
                    p_scnt = pending.get("scnt", scnt)
                    if p_svc in stock and p_scnt in stock.get(p_svc, {}):
                        stock[p_svc][p_scnt].insert(0, old_num)
                        save_stock()
                    print(f"[N:] Cancelled {old_num} for user {uid_n} → restored to stock")
                try:
                    bot.delete_message(call.message.chat.id, call.message.message_id)
                except Exception:
                    pass
            else:
                # ── OTP already received (or first country select) → strip buttons only ──
                try:
            ...
            if scnt in stock.get(svc, {}) and stock[svc][scnt]:
                ...
                if old_nums:
                    _save_user_map()
                    print(f"[N:] Released stale number(s) {old_nums} for user {uid_n}")
                ...
                # Initialise pending record (num_msg_id filled after send)
                _user_pending[uid_n] = {
                    "number": num,
                    "num_msg_id": None,
                    "has_otp": False,
                ...
                _user_pending[uid_n]["num_msg_id"] = new_msg.message_id
